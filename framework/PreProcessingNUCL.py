...
def nuclei_segmentation(image,name,dir_nucldec,dir_masks,algorithm,algorithm_specs):
    ...
    if len(image.shape) == 2:
        ...
        dim = 3
    elif len(image.shape) == 3 and name[-8] == 0:
        dim = 2
    else:
        print(">>>>>> SEGMENTATION: Image " + str(name) + str(" error."))
        return
    
    print(">>>>>> SEGMENTATION: Image " + str(name))
...
#             labels_ = watershed(-distance, markers, mask=labels_)
    ...
    imwrite(os.path.join(dir_masks, name), final,photometric='minisblack')
...
def nuclei_preprocessing(image,name,dir_masks,plot,save):
